...
def segment_automatic_with_export(preprocess_json_path, checkpoint_path, format="rle", device="cuda"):
    ...
    # 7. Visualiser
    visualize_with_ids(image, masks, output_file=f"{base_name}_sam_visualization.png")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("\n❌ Usage : python sam_export_json.py <preprocess_json> [format]")
        print("\nExemples :")
        print("  python sam_export_json.py IMG_5177-535x356_preprocessed.json")
        print("  python sam_export_json.py IMG_5177-535x356_preprocessed.json rle")
        print("  python sam_export_json.py IMG_5177-535x356_preprocessed.json binary")
        print("\n⚠️  IMPORTANT : Vous devez d'abord lancer preprocess_image.py")
        return
    
    preprocess_json_path = sys.argv[1]
    ...
    # Vérifier que le fichier preprocess existe
    if not os.path.exists(preprocess_json_path):
        print(f"❌ Fichier preprocess non trouvé : {preprocess_json_path}")
        print("   Vous devez d'abord lancer : python preprocess_image.py <image>")
        return
    ...
    if choice == '1':
        result = segment_automatic_with_export(preprocess_json_path, checkpoint_path, format_type, device)
    elif choice == '2':
        result = segment_interactive_with_export(preprocess_json_path, checkpoint_path, format_type, device)
    else:
